...
local Watcher = CreateFrame("Frame", "Watcher")
Watcher.Watchers = {}
...
-- local defaults = {}
local colors = I.Colors.Classes
local watcherPrototype = CreateFrame("Frame", "watcherPrototype")
...
function Watcher:UpdateAlpha(elapsed)
...
local function Flash(self)
    ...
    if time >0 and time < 5 then
        self:SetAlpha(Watcher.AlphaFrame.BuffAlphaValue)
    else
        self:SetAlpha(1)
    end
end
...
function watcherPrototype:CreateButton(mode)
    ...
    if mode=="BAR" then
        ...
        button.statusbar:SetStatusBarColor(colors[I.MyClass].r, colors[I.MyClass].g, colors[I.MyClass].b, 1)
        button.statusbar:SetReverseFill(true)
        ...
        spark:SetAlpha(.8)
        spark:SetPoint("TOPRIGHT", button.statusbar:GetStatusBarTexture(), "TOPLEFT", 10, 13)
        spark:SetPoint("BOTTOMLEFT", button.statusbar:GetStatusBarTexture(), "BOTTOMLEFT", -10, -13)
        button.time = button:CreateFontString(nil, "OVERLAY")
        button.time:SetFont(watcherFont.path, watcherFont.size, watcherFont.flag)
        button.time:SetPoint("BOTTOMRIGHT", button.statusbar, "TOPRIGHT", 0, 2)
        button.time:SetText("60")
        button.name = button:CreateFontString(nil, "OVERLAY")
        button.name:SetFont(watcherFont.path, watcherFont.size, watcherFont.flag)
        ...
        -- button.cooldown.SetHideCountdownNumbers = function() return end
        button.mode = "ICON"
    end
    ...
    button.count:SetFont(watcherFont.path, watcherFont.size * (I:Round(self.size) / 30), watcherFont.flag)
    button.count:SetPoint("BOTTOMRIGHT", button , "BOTTOMRIGHT", 4, -4)
    button.count:SetJustifyH("RIGHT")

    button.value = textFrame:CreateFontString(nil, "OVERLAY")
    button.value:SetFont(watcherFont.path, ( watcherFont.size - 3 ) * (I:Round(self.size) / 30), watcherFont.flag)
...
function watcherPrototype:OnEvent(event, unit)
    if event == "PLAYER_ENTERING_WORLD" then
        ...
        -- I:CreateMover(self.holder, self.name.."Holder", self.name, true)
...
function Watcher:Enable()
    ...
    if type(IC["Watcher"]["filters"][I.MyClass]) == "table" then
        for _, t in ipairs(IC["Watcher"]["filters"][I.MyClass]) do
            self:NewWatcher(t)
        end
    end
    if type(IC["Watcher"]["filters"]["ALL"]) == "table" then
        for _, t in ipairs(IC["Watcher"]["filters"]["ALL"]) do
            self:NewWatcher(t)
        end
    end
    wipe(IC["Watcher"]["filters"])
...
function Watcher:NewWatcher(data)
    local name, module
    ...
    if module.CD or module.itemCD then
        ...
        module:RegisterEvent("SPELL_UPDATE_USABLE")
    end
    module:SetScript("OnEvent", module.OnEvent)
    Watcher.Watchers[module.name] = module
end
