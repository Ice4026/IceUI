...
local function SkinDBM()
			...
			if not bar.injected then
				function bar:ApplyStyle()
					...
					if not texture.styled then
						texture:SetTexture(IC.Medias.Textures.normal)
						texture.styled=true
					end

					tbar:SetStatusBarTexture(IC.Medias.Textures.normal)
					...
					if not name.styled then
						...
						name:SetWidth(165)
						name:SetHeight(8)
						name:SetFont(IC.Medias.Textures.font, 12, "OUTLINE")
					...
					if not timer.styled then
						...
						timer:SetFont(IC.Medias.Textures.font, 12, "OUTLINE")
	...
	local function SkinBossTitle()
		...
		if not anchor.styled then
			...
			if header[1]:IsObjectType("FontString") then
				header[1]:SetFont(IC.Medias.Textures.font, 12, "OUTLINE")
	...
	local function SkinBoss()
		...
		while (_G[format("DBM_BossHealth_Bar_%d", count)]) do
			...
			if not progress.styled then
				progress:SetStatusBarTexture(IC.Medias.Textures.normal)
	...
	local function SkinRange(self, range, filter, forceshow, redCircleNumPlayers)
		...
		if not self.styled then
			...
			DBMRangeCheckRadar.text:SetFont(IC.Medias.Textures.font, IC.Medias.Textures.fontsize, IC.Medias.Textures.fontflag)
	...
	hooksecurefunc(DBM, "ShowUpdateReminder", function()
		for i = UIParent:GetNumChildren(), 1, -1 do
			...
			if editBox and editBox:GetObjectType() == "EditBox" and editBox:GetText() == "http://www.deadlybossmods.com" and not frame.styled then
				...
				local bg = Skins:CreateBDFrame(editBox, .25)
				bg:SetPoint("TOPLEFT", -2, -6)
				bg:SetPoint("BOTTOMRIGHT", 2, 8)

				Skins:Reskin(select(2, frame:GetChildren()))
...
Skins:RegisterSkin("DBM-Core", SkinDBM)
...
local function SkinGUI()
	...
	if DBM_GUI_OptionsFramecheck then
		Skins:Reskin(DBM_GUI_OptionsFramecheck)
		Skins:Reskin(DBM_GUI_OptionsFramecheck2)
	...
	local function styleDBM()
		...
		while option do
			...
			if objType == "CheckButton" then
				Skins:ReskinCheck(option)
			elseif objType == "Slider" then
				Skins:ReskinSlider(option)
			elseif objType == "EditBox" then
				Skins:ReskinInput(option)
			elseif option:GetName():find("DropDown") then
				Skins:ReskinDropDown(option)
	...
	Skins:SetBD(DBM_GUI_OptionsFrame)
	Skins:Reskin(DBM_GUI_OptionsFrameOkay)
	Skins:Reskin(DBM_GUI_OptionsFrameWebsiteButton)
	Skins:ReskinScroll(DBM_GUI_OptionsFramePanelContainerFOVScrollBar)
	Skins:ReskinScroll(DBM_GUI_OptionsFrameBossModsListScrollBar)
end

Skins:RegisterSkin("DBM-GUI", SkinGUI)
