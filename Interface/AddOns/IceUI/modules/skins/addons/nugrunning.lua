...
local function SkinNugRunning()
	...
	function NugRunning.ConstructTimerBar(w, h)
		...
		f.bar:SetStatusBarTexture(IC.Medias.Textures.normal)
		f.bar:GetStatusBarTexture():SetDrawLayer("ARTWORK")

		-- Fonts
		do
			f.timeText:SetFont(IC.Medias.Textures.font, IC.Medias.Textures.fontsize, "OUTLINE")
			f.spellText:SetFont(IC.Medias.Textures.font, IC.Medias.Textures.fontsize, "OUTLINE")
			f.spellText:SetAlpha(1)
			f.stacktext:SetFont(IC.Medias.Textures.font, IC.Medias.Textures.fontsize, "OUTLINE")
		end
		...
		if amount > 1 and amount <= visualstacksmax then
			...
			for i = 1, amount do
				if not stacks.tex[i] then
					...
					tex:SetTexture(IC.Medias.Textures.normal)
					tex:Point("TOP", 0, -1)
...
Skins:RegisterSkin("NugRunning", SkinNugRunning)