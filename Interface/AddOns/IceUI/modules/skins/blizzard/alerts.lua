...
local function LoadSkin()
		...
		if frame then
			if not frame.bg then
				...
				if not frame.Icon.Texture.b then
					frame.Icon.Texture.b = Skins:CreateBG(frame.Icon.Texture)
				end

				frame:HookScript("OnEnter", function()
					Skins:CreateBD(frame.bg)
				end)
		...
		if frame then
			if not frame.bg then
				...
				if not frame.dungeonTexture.b then
					...
					Skins:CreateBD(frame.dungeonTexture.b, 0)
				end
		...
		if frame then
			if not frame.bg then
				...
				if not ScenarioAlertFrame1DungeonTexture.b then
					...
					Skins:CreateBD(ScenarioAlertFrame1DungeonTexture.b, 0)
				end
		...
		if frame then
			if not frame.bg then
				...
				if not frame.Icon.Texture.b then
					...
					Skins:CreateBD(frame.Icon.Texture.b, 0)
				end
				frame.Icon.Texture:SetTexCoord(.08, .92, .08, .92)
	...
	hooksecurefunc(LootUpgradeAlertSystem, "setUpFunction", function(frame)
		...
		frame.BaseQualityBorder:SetTexture(IC.Medias.Textures.gloss)
		frame.UpgradeQualityBorder:SetTexture(IC.Medias.Textures.gloss)
