...
local function LoadSkin()
	...
	for i = 1, 4 do
		for j = 1, 3 do
			Skins:Reskin(_G["StaticPopup"..i.."Button"..j])
		end
		...
		Skins:ReskinInput(_G["StaticPopup"..i.."EditBox"], 20)
	end
	...
	Skins:ReskinClose(RolePollPopupCloseButton)
	Skins:ReskinClose(ItemRefCloseButton)
	Skins:ReskinClose(FloatingBattlePetTooltip.CloseButton)
	...
	for i = 1, #FrameBDs do
		...
		Skins:CreateBD(FrameBD)
		Skins:CreateSD(FrameBD)
	end
	...
	Skins:CreateBD(PVPReadyDialog)
	PVPReadyDialog.SetBackdrop = R.dummy
	Skins:CreateSD(PVPReadyDialog)

	Skins:Reskin(PVPReadyDialog.enterButton)
	Skins:Reskin(PVPReadyDialog.leaveButton)
	Skins:ReskinClose(PVPReadyDialogCloseButton)
	...
	local function SkinDropDownList(level, index)
		for i = 1, UIDROPDOWNMENU_MAXLEVELS do
			...
			if not backdrop.reskinned then
				Skins:CreateBD(menu)
				Skins:CreateBD(backdrop)
				Skins:CreateStripesThin(menu)
			...
			if bu:IsShown() and x then
				...
				if not bu.notCheckable then
					...
					if co == 0 then
						...
						check:SetTexture(IC.Medias.Textures.gloss)
	...
	Skins:CreateBD(AudioOptionsSoundPanelPlayback, .25)
	Skins:CreateBD(AudioOptionsSoundPanelHardware, .25)
	Skins:CreateBD(AudioOptionsSoundPanelVolume, .25)
	Skins:CreateBD(AudioOptionsVoicePanelTalking, .25)
	Skins:CreateBD(AudioOptionsVoicePanelBinding, .25)
	Skins:CreateBD(AudioOptionsVoicePanelListening, .25)
	...
	for i = 1, #dropdowns do
		Skins:ReskinDropDown(_G[dropdowns[i]])
	end
	...
	for i = 1, #sliders do
		Skins:ReskinSlider(_G[sliders[i]])
	end
	...
	Skins:Reskin(RecordLoopbackSoundButton)
	Skins:Reskin(PlayLoopbackSoundButton)
	Skins:Reskin(AudioOptionsVoicePanelChatMode1KeyBindingButton)
	...
	Skins:Reskin(CompactUnitFrameProfilesSaveButton)
	Skins:Reskin(CompactUnitFrameProfilesDeleteButton)
	Skins:Reskin(CompactUnitFrameProfilesGeneralOptionsFrameResetPositionButton)
	Skins:Reskin(InterfaceOptionsHelpPanelResetTutorials)
	...
	--实名好友弹窗位置
	Skins:ReskinClose(BNToastFrameCloseButton)
	...
	--[[ if IsMacClient() then
		Skins:CreateBD(MacOptionsFrame)
		MacOptionsFrameHeader:SetTexture("")
		MacOptionsFrameHeader:ClearAllPoints()
		MacOptionsFrameHeader:SetPoint("TOP", MacOptionsFrame, 0, 0)

		Skins:CreateBD(MacOptionsFrameMovieRecording, .25)
		Skins:CreateBD(MacOptionsITunesRemote, .25)

		Skins:Reskin(MacOptionsButtonKeybindings)
		Skins:Reskin(MacOptionsButtonCompress)
		Skins:Reskin(MacOptionsFrameCancel)
		Skins:Reskin(MacOptionsFrameOkay)
		Skins:Reskin(MacOptionsFrameDefaults)

		Skins:ReskinDropDown(MacOptionsFrameResolutionDropDown)
		Skins:ReskinDropDown(MacOptionsFrameFramerateDropDown)
		Skins:ReskinDropDown(MacOptionsFrameCodecDropDown)
		for i = 1, 10 do
			if _G["MacOptionsFrameCheckButton"..i] then
				Skins:ReskinCheck(_G["MacOptionsFrameCheckButton"..i])
			end
		end
		Skins:ReskinSlider(MacOptionsFrameQualitySlider)
	...
	Skins:ReskinPortraitFrame(AddonList, true)
	Skins:Reskin(AddonListEnableAllButton)
	Skins:Reskin(AddonListDisableAllButton)
	Skins:Reskin(AddonListCancelButton)
	Skins:Reskin(AddonListOkayButton)
	Skins:ReskinCheck(AddonListForceLoad)
	Skins:ReskinDropDown(AddonCharacterDropDown)
	Skins:ReskinScroll(AddonListScrollFrameScrollBar)

	AddonCharacterDropDown:SetWidth(170)

	for i = 1, MAX_ADDONS_DISPLAYED do
		Skins:ReskinCheck(_G["AddonListEntry"..i.."Enabled"])
		Skins:Reskin(_G["AddonListEntry"..i.."Load"])
	end
	...
	hooksecurefunc("NavBar_AddButton", function(self, buttonData)
		...
		if not navButton.restyled then
			...
			tex:SetTexture(Skins["media"].arrowDown)
			tex:SetSize(8, 8)
			tex:SetPoint("CENTER")
			arrowButton.tex = tex
	...
	Skins:Reskin(SplashFrame.BottomCloseButton)
	Skins:ReskinClose(SplashFrame.TopCloseButton)
