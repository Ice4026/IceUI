...
local function LoadSkin()
	...
	Skins:SetBD(CharacterFrame)
	Skins:SetBD(PetStableFrame)
	local frames = {
		"CharacterFrame",
		"CharacterFrameInset",
	...
	Skins:ReskinClose(CharacterFrameCloseButton)
	Skins:ReskinScroll(CharacterStatsPaneScrollBar)
	Skins:ReskinScroll(PaperDollTitlesPaneScrollBar)
	Skins:ReskinScroll(PaperDollEquipmentManagerPaneScrollBar)
	Skins:ReskinScroll(ReputationListScrollFrameScrollBar)
	Skins:ReskinScroll(GearManagerDialogPopupScrollFrameScrollBar)
	Skins:ReskinArrow(CharacterFrameExpandButton, "left")
	Skins:Reskin(PaperDollEquipmentManagerPaneEquipSet)
	Skins:Reskin(PaperDollEquipmentManagerPaneSaveSet)
	Skins:Reskin(GearManagerDialogPopupOkay)
	Skins:Reskin(GearManagerDialogPopupCancel)
	Skins:ReskinClose(ReputationDetailCloseButton)
	Skins:ReskinCheck(ReputationDetailAtWarCheckBox)
	Skins:ReskinCheck(ReputationDetailInactiveCheckBox)
	Skins:ReskinCheck(ReputationDetailMainScreenCheckBox)
	Skins:ReskinCheck(ReputationDetailLFGBonusReputationCheckBox)
	Skins:ReskinInput(GearManagerDialogPopupEditBox)
	...
	for i = 1, 4 do
		Skins:CreateTab(_G["CharacterFrameTab"..i])
	end
	...
	local function UpdateFactionSkins()
		for i = 1, GetNumFactions() do
			...
			if statusbar then
				statusbar:SetStatusBarTexture(IC.Medias.Textures.gloss)

				if not statusbar.reskinned then
				--	Skins:CreateBD(statusbar, .25)
					local frame = CreateFrame("Frame",nil, statusbar)
					Skins:CreateBD(frame, .25)
	...
	if R.myclass == "HUNTER" or R.myclass == "MAGE" or R.myclass == "DEATHKNIGHT" or R.myclass == "WARLOCK" then
		if R.myclass == "HUNTER" then
			...
			Skins:CreateBD(bd, .25)

			Skins:ReskinClose(PetStableFrameCloseButton)
			Skins:ReskinArrow(PetStablePrevPageButton, "left")
			Skins:ReskinArrow(PetStableNextPageButton, "right")
